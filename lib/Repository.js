...
class TenantRepository {
    ...
    async init() {
        this.currentDb = await this.connectionResolver();
    ...
    async attachDbContext(modelsPath, connectionString, connectionID)
    {
        ...
        // setup a new Sequekize connection.
    ...
    async connectionResolver() {
        ...
        return await new Promise((resolve, reject) => {
            try {
                em.on('requestUrl', async (data) => {
                    ...
                    try {
                        ...
                        if ((domain.toLowerCase() === process.env.TENANCY_DEFAULT_HOSTNAME.toLowerCase()) || domain.toLowerCase() === 'default') {
                            ...
                            this.db['default'].sequelize.query(`select uuid from hostnames where fqdn = '${domain}'`)
                                .then((result) => {
                                    return resolve(result[0][0].uuid);
                                });
                        }
                    } catch (e) {
                ...
                return reject('DB', `Error while working with emitted event for "${domain}"`, e);
            } finally {
                return resolve('default');
              }
        })
     ...
     */
    async currentDB() {
        console.log(`${this.currentDb} = `, this.db[this.currentDb].sequelize.config);
        return this.db[this.currentDb].sequelize;
    }
