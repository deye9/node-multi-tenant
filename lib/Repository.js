...
class TenantRepository {
    ...
    constructor(modelType) {
        let db = this.connectionResolver();
    ...
    connectionResolver() {
        ...
        global.em.on('requestUrl', async (data) => {
            let requestUrl = data,
                domain = requestUrl,
                // connectionString = '',
            ...
            try {
                ...
                if (domain.toLowerCase() === process.env.TENANCY_DEFAULT_HOSTNAME.toLowerCase()) {
                    connectionString = `${db.sequelize.options.dialect}://${db.sequelize.options.username}:${db.sequelize.options.password}@${db.sequelize.options.host}:${db.sequelize.options.port}/${db.sequelize.options.database}`;
                } else {
                    const result = await this.findOne({
                        "fqdn": domain
                    });
                    connectionString = `${db.sequelize.options.dialect}://${db.sequelize.options.username}:${db.sequelize.options.password}@${db.sequelize.options.host}:${db.sequelize.options.port}/${result.dataValues.uuid}`;
                }
            } catch (e) {
                return new RepositoryError('DB', `Unable to retrieve UUID detail for Tenant "${domain}"`, e);
            }
        });
