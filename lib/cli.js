...
cli.tenancy.install = async() => {
  ...
  cli.executeCommand(`mkdir -p ${migrationPath}/tenants`, false);

  console.log("Moving existing migrations to the migrations/tenants folder.");
  cli.executeCommand(`mv ${migrationPath}/*.js ${migrationPath}/tenants`, false);

  console.log('Copying tenancy migration files to the migrations folder');
  cli.executeCommand(`cp node_modules/node_multi_tenant/dist/migrations/*.js ${migrationPath}/`, false);
  
  console.log("Copying tenancy model files to the models folder");
  cli.executeCommand(`cp node_modules/node_multi_tenant/dist/models/*.js ${modelsPath}/`, false);

  console.log("Attempting to drop existing database. Kindly ignore message if database does not exists");
  await cli.executeCommand("node_modules/.bin/sequelize db:drop");

  console.log("Attempting to create the database");
  await cli.executeCommand("node_modules/.bin/sequelize db:create");

  console.log("Performing Migrations");
  await cli.executeCommand("node_modules/.bin/sequelize db:migrate");

  console.log("Seeding all database");
  await cli.executeCommand("node_modules/.bin/sequelize db:seed:all");
...
cli.executeCommand = async (cliCommand, displayline=true) => {
  dir = await exec(cliCommand, async function(err, stdout, stderr) {
    if (err) {
