...
const util = require('util'),
  ...
  {files: fileReader} = require('./fileHandler'),
 ...
 */
executeCommand = async (cliCommand) => {
...
cli.tenancy.install = async() => {
  ...
  await executeCommand(`mkdir -p ${migrationPath}/tenants`);

  console.log("Moving existing migrations to the migrations/tenants folder.");
  await executeCommand(`mv ${migrationPath}/*.js ${migrationPath}/tenants`);

  console.log("Copying tenancy migration files to the migrations folder");
  await executeCommand(`cp node_modules/node_multi_tenant/dist/migrations/*.js ${migrationPath}/`);

  console.log("Copying tenancy model files to the models folder");
  await executeCommand(`cp node_modules/node_multi_tenant/dist/models/*.js ${modelsPath}/`);

  console.log("Attempting to drop existing database. Kindly ignore message if database does not exists");
  await executeCommand("node_modules/.bin/sequelize db:drop");

  console.log("Attempting to create the database");
  await executeCommand("node_modules/.bin/sequelize db:create");

  console.log("Performing Migrations");
  await executeCommand("node_modules/.bin/sequelize db:migrate");
  await executeCommand(`node_modules/.bin/sequelize db:migrate --migrations-path=${migrationPath}/tenants`);

  console.log("Seeding all database");
  await executeCommand("node_modules/.bin/sequelize db:seed:all");
...
cli.tenancy.unseed = (undoAction) => {
  ...
  executeCommand("node_modules/.bin/sequelize db:seed:undo");
  } else {
  // undo all seeds
  executeCommand("node_modules/.bin/sequelize db:seed:undo:all");
  }
};
...
cli.tenancy.rollback = () => {
  executeCommand("node_modules/.bin/sequelize db:migrate:undo");
};
...
// executeCommand = (cliCommand, displayline=true) => {
